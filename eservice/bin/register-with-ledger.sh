...
function Register {
    if [ ! -f $eservice_enclave_info_file ]; then
	    ...
	    then
            try ${SRCDIR}/ccf_transaction_processor/scripts/register_enclave_expected_measurements.py --logfile __screen__ --loglevel INFO \
                --mrenclave ${VAR_MRENCLAVE} --basename  ${VAR_BASENAME} --ias-public-key "$(cat $PDO_IAS_KEY_PEM)"
        else
