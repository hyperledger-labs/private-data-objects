...
namespace ccfapp
{
    ...
    TPHandlerRegistry ::TPHandlerRegistry (AbstractNodeContext& context):
        ...
        enclave_expected_measurements("expected_measurements"),
        ...
        signer("signer")
    {
        ...
        auto register_enclave_expected_measurements = [this](auto& ctx, const nlohmann::json& params) {

            const auto in = params.get<RegisterEnclaveExpectedMeasurements::In>();

            // get the current measurements view
            auto measurements_view = ctx.tx.rw(enclave_expected_measurements);

            // collect the data to be stored
            EnclaveExpectedMeasurements expected_measurements;

            expected_measurements.mrenclave = in.mrenclave;
            expected_measurements.basename = in.basename;
            expected_measurements.ias_public_key = in.ias_public_key;

            //store the data
            measurements_view->put(PDO_ENCLAVE_EXPECTED, expected_measurements);
        ...
        make_endpoint(
            REGISTER_ENCLAVE_EXPECTED_MEASUREMENTS,
            HTTP_POST,
            json_adapter(register_enclave_expected_measurements),
            member_cert_sign_required).install();
