...
quote_status_e get_quote_status(const char* ias_report, unsigned int ias_report_len)
{
    bool ret = false;
    json root;
    std::string ias_quote_status_str;
    int i;

    CATCH(ret, root = json::parse(ias_report));
    COND2LOGERR(!ret, "invalid ias report json");

    CATCH(ret, ias_quote_status_str = root[IAS_QUOTE_STATUS_JSON_STRING].template get<std::string>());
    COND2LOGERR(!ret, "invalid ias quote status field");

    for (i = 1; i < QS_NUMBER; i++)
    {
        if (0 == ias_quote_status_str.compare(quote_status[i].s))
        {
...
verify_status_t verify_ias_certificate_chain(const char* cert_pem, const time_t untrusted_time)
{
    ...
    COND2ERR(param == NULL);
    X509_VERIFY_PARAM_set_time(param, untrusted_time);
