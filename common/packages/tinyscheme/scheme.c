...
INTERFACE static pointer set_vector_elem(pointer vec, int ielem,
					 pointer a);
...
static pointer port_from_string(scheme * sc, char *start,
...
static void assign_proc(scheme * sc, enum scheme_opcodes,
			const char *name);
...
static pointer _Error_1(scheme * sc, const char *s, pointer a)
{
	...
	if (a != 0) {
	    sc->code =
		cons(sc, cons(sc, sc->QUOTE, cons(sc, (a), sc->NIL)),
		     sc->NIL);
	} else {
	    sc->code = sc->NIL;
	}
...
static pointer opexe_0(scheme * sc, enum scheme_opcodes op)
{
	...
	{
	    pointer f =
		find_slot_in_env(sc, sc->envir, sc->COMPILE_HOOK, 1);
	    if (f == sc->NIL) {
		sc->value = sc->code;
		/* Fallthru */
	    } else {
...
static pointer opexe_5(scheme * sc, enum scheme_opcodes op)
{
	...
	case TOK_SHARP:{
		pointer f =
		    find_slot_in_env(sc, sc->envir, sc->SHARP_HOOK, 1);
		if (f == sc->NIL) {
...
static void Eval_Cycle(scheme * sc, enum scheme_opcodes op)
{
	...
	if (pcd->name != 0) {	/* if built-in function, check arguments */
		...
		snprintf(msg, STRBUFFSIZE, "%s: needs%s %d argument(s)",
			 pcd->name,
			 pcd->min_arity ==
			 pcd->max_arity ? "" : " at most", pcd->max_arity);
	    }
...
static void assign_proc(scheme * sc, enum scheme_opcodes op,
			const char *name)
{
    pointer x, y;
...
INTERFACE static pointer s_immutable_cons(scheme * sc, pointer a,
					  pointer b)
{
			     ...
			     func_dealloc free)
{
    ...
    car(sc->sink) = sc->NIL;
    /* init EOF */
    typeflag(sc->EOF_OBJ) = (T_EOF | MARK);
    car(sc->EOF_OBJ) = cdr(sc->EOF_OBJ) = sc->NIL;
...
void scheme_read_string(scheme * sc, const char *cmd, size_t cmdlen)
{
    ...
    sc->load_stack[0].rep.string.past_the_end = (char *) cmd + cmdlen;
    ...
    sc->inport = sc->loadport;
    sc->args = mk_integer(sc, sc->file_i);
    Eval_Cycle(sc, OP_READ_INTERNAL);
    typeflag(sc->loadport) = T_ATOM;
    if (sc->retcode == 0) {
	sc->retcode = sc->nesting != 0;
    }
...
void scheme_load_string(scheme * sc, const char *cmd, size_t cmdlen)
{
    dump_stack_reset(sc);
    sc->envir = sc->global_env;
    sc->file_i = 0;
...
void scheme_safe_load_string(scheme * sc, const char *cmd, size_t cmdlen)
{
    dump_stack_reset(sc);
...
void scheme_load_string_o(scheme * sc, const char *cmd, size_t cmdlen)
{
    dump_stack_reset(sc);
    sc->envir = sc->global_env;
    sc->file_i = 0;
...
void scheme_define(scheme * sc, pointer envir, pointer symbol,
		   pointer value)
{
    pointer x;
...
void scheme_register_foreign_func(scheme * sc, scheme_registerable * sr)
{
    scheme_define(sc,
		  sc->global_env,
		  mk_symbol(sc, sr->name), mk_foreign_func(sc, sr->f));
}
...
pointer scheme_call(scheme * sc, pointer func, pointer args)
{
    ...
    /* this little gem is necessary if we want to use string ports in the contracts */
    /* reading from the port pushes the old port and the interpreter fails when it */
    /* restores the old port */
    sc->inport = port_from_string(sc, cmd, cmd, port_input);
...
INTERFACE pointer scheme_find_symbol_value(scheme * sc, pointer env,
					   pointer symbol)
{
    return find_slot_in_env(sc, env, symbol, 1);
}

INTERFACE pointer scheme_find_symbol(scheme * sc, const char *name)
{
    return oblist_find_by_name(sc, name);
}
